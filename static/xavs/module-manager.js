...
class XAVSModuleManager {
        ...
        const modules = [
            {
                ...
                path: '../static/xavs-modules/welcome.js',
                icon: '🏠',
                order: 1
            },
            {
                ...
                path: '../static/xavs-modules/app.js',
                icon: '🚀',
                order: 2
            },
            {
            ...
            },
            {
                ...
                path: '../static/xavs-modules/xavs-storage.js',
                icon: '💾',
                order: 5
            },
            {
            ...
            },
            {
                ...
                path: '../static/xavs-modules/xavs-images.js',
                icon: '📦',
                order: 7
            },
            {
    ...
    async loadInitialModule() {
        const workflowStatus = await window.xavsState.getWorkflowStatus();
    ...
    async loadModule(moduleId) {
        try {
            ...
            const canNavigate = await window.xavsState.navigateToModule(moduleId);
            ...
            try {
                ...
                await moduleInstance.initialize(this.container, window.xavsState);
            ...
            <div class="nav-header">
                ...
                <div class="progress-indicator">
                    <div class="progress-bar">
                        <div class="progress-fill" style="width: ${workflowStatus && workflowStatus.progress ? workflowStatus.progress : 0}%"></div>
                    </div>
                    <span class="progress-text">${workflowStatus && workflowStatus.progress ? workflowStatus.progress : 0}% Complete</span>
                </div>
            </div>
    ...
    renderNavItem(module, workflowStatus) {
        const isCompleted = workflowStatus && workflowStatus.completedSteps && workflowStatus.completedSteps.includes(module.id) || false;
        const isCurrent = workflowStatus && workflowStatus.currentStep === module.id;
    ...
    canAccessModule(moduleId, workflowStatus) {
        ...
        for (let i = 0; i < moduleIndex; i++) {
            ...
            if (!(workflowStatus && workflowStatus.completedSteps && workflowStatus.completedSteps.includes(prerequisite.id))) {
                return false;
            }
        }
     ...
     */
    async showAuditLog() {
        const logs = await window.xavsState.getAuditLogs(50);
    ...
    async exportState() {
        try {
            const exportData = await window.xavsState.exportState();
...
window.XAVSModuleManager = XAVSModuleManager;
window.xavsModules = new XAVSModuleManager();
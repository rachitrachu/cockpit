...
class XAVSStateManager {
    ...
    canProceedToNextStep(state) {
        ...
        return state.modules[previousStep] && state.modules[previousStep].completed || false;
    }
    ...
    async navigateToModule(moduleId) {
        ...
        for (let i = 0; i < moduleIndex; i++) {
            ...
            if (!state.modules[prerequisite] || !state.modules[prerequisite].completed) {
    ...
    async auditLog(action, details = {}) {
        try {
            ...
            const auditEntry = {
                ...
                action,
                details,
                sessionId: (cockpit.transport && cockpit.transport.options && cockpit.transport.options.channel) || 'unknown'
            };
...
window.XAVSStateManager = XAVSStateManager;
window.xavsState = new XAVSStateManager();